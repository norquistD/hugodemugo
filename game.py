...
def GameLoop():
    ...
    dir_key = 1  # start moving 'right' logically even if not yet moving
    dir_from_str = {'left': 0, 'right': 1, 'up': 2, 'down': 3}
    ...
    while not dead:
        ...
        action = learner.act(dir_key, snake_list, (foodx,foody))
        if action == "left":
            x1_change = -BLOCK_SIZE
            y1_change = 0
            dir_key = dir_from_str["left"]
        elif action == "right":
            x1_change = BLOCK_SIZE
            y1_change = 0
            dir_key = dir_from_str["right"]
        elif action == "up":
            y1_change = -BLOCK_SIZE
            x1_change = 0
            dir_key = dir_from_str["up"]
        elif action == "down":
            y1_change = BLOCK_SIZE
            x1_change = 0
            dir_key = dir_from_str["down"]
...
while True:
    ...
    print(f"Games: {game_count}; Score: {score}; Reason: {reason}; Current highscore: {high_score}") # Output results of each game to console to monitor as agent is training
    if score > high_score:
        high_score = score
        learner.SaveQvalues(path="highscores/highscore"+str(high_score)+".json")
    game_count += 1
